...
pub use orbit_research_common::legacy_contract::RepositoryId;
...
pub enum ImportError {
    ...
    #[error(transparent)]
    Contract(#[from] orbit_research_common::legacy_contract::ContractError),
    #[error(transparent)]
...
fn has_forbidden_component(path: &Path) -> bool {
    path.components()
        .any(|c| matches!(c.as_os_str().to_str(), Some(".git") | Some(".orbit")))
}
...
fn walk_glob(dir: &Path, components: &[&str], results: &mut Vec<PathBuf>) {
    ...
    if *first == "**" {
        ...
        let mut subdirs: Vec<PathBuf> = entries
            ...
            .filter(|p| p.is_dir())
            .filter(|p| {
                !has_forbidden_component(p.file_name().map(Path::new).unwrap_or(Path::new("")))
            })
            .collect();
        subdirs.sort();
        for sub in subdirs {
...
fn make_record(
    ...
    let revision_id = orbit_research_common::legacy_contract::revision_digest(&record)?;
...
impl Importer {
    ...
    fn provenance(&mut self, path: &Path, selector: &str) -> Result<Value, ImportError> {
        ...
        let blob = self
            .revision
            .as_ref()
            .and_then(|rev| git::run(&self.root, &["rev-parse", &format!("{rev}:{rel}")]));
    ...
    fn source_artifact(
        ...
        self.record(
            ...
            payload,
            raw,
            RecordOpts::new().selector(selector),
        )
    }
    ...
    fn item(
        ...
        for record in records {
            let errors = orbit_research_common::legacy_contract::validate(&record, &[]);
            if !errors.is_empty() {
    ...
    fn json_file(&mut self, path: &Path, raw: Value) -> Result<(), ImportError> {
        ...
        let is_dataset = self.adapter == "astrolabe"
        ...
        if is_claims_json {
            ...
            let semantic_digest =
                orbit_research_common::legacy_contract::protocol_digest(&semantic)?;
            let payload = json!({
                "semantic": semantic,
            ...
            let role = if self.adapter == "orrery" {
                "result"
            } else {
                "source"
            };
    ...
    fn claims(&mut self, path: &Path, raw: Value) -> Result<(), ImportError> {
        ...
        if let (Some(doc), true) = (doc, title_is_str) {
            ...
            let rec = self.record(
                ...
                payload,
                raw.clone(),
                RecordOpts::new().activity(activity),
            )?;
            records.push(rec);
        }
        self.item(
        ...
        for (n, claim) in claims_list.iter().enumerate() {
            ...
            let scope = if kind == Some("derived") {
                "derivation"
            } else {
                "unknown"
            };
            let role = if kind == Some("postulate") {
                "postulate"
            } else {
                "claim"
            };
    ...
    fn dataset(&mut self, path: &Path, raw: Value) -> Result<(), ImportError> {
        ...
        let missing_snapshot: &[&str] = if snapshot.is_some() {
            &[]
        } else {
            &["dataset-snapshot"]
        };
        ...
        let mut rec = self.record(
            ...
            payload,
            raw.clone(),
            RecordOpts::new().missing(missing_snapshot),
        )?;
        ...
        let new_digest = orbit_research_common::legacy_contract::revision_digest(&rec)?;
    ...
    fn markdown(&mut self, path: &Path, content: String) -> Result<(), ImportError> {
        ...
        for (idx, line) in lines.iter().enumerate() {
            ...
            if cells.len() < 4 || cells[2].trim().is_empty() {
                ...
                let payload =
                    json!({"role": "hypothesis", "statement": statement, "domain": "empirical"});
                let rec = self.record(
                    path,
                    "claim",
                    &ident,
                    ...
                    RecordOpts::new()
                        .selector(loc.clone())
                        .scope("observation")
                        .missing(&["verdict", "activity", "protocol"]),
                )?;
                ...
                self.item(
                    ...
                    vec![(
                        ...
                        "R/E row retained; design prose does not establish a run or its outcome."
                            .to_string(),
                    )],
                );
            }
            ...
            if heading_re.is_match(line) {
                self.item(
                    ...
                    vec![(
                        ...
                        "Definition/discussion heading retained with its full source document."
                            .to_string(),
                    )],
                );
            }
    ...
    fn parallax_document(&mut self, path: &Path, content: &str) -> Result<bool, ImportError> {
        ...
        for line in front_body.lines() {
            ...
            let value = orbit_research_common::legacy_contract::parse_json(value_str.as_bytes())
        ...
        if let Some(eid) = eid {
            ...
            let protocol = self.record(
                ...
                RecordOpts::new()
                    ...
                    .scope("observation")
                    .missing(&["independent-freeze-chronology"]),
            )?;
            let execution = status
                .filter(|s| {
                    matches!(
                        *s,
                        "completed" | "failed" | "cancelled" | "running" | "planned"
                    )
                })
                .unwrap_or("unknown");
            ...
            let run = self.record(
                ...
                RecordOpts::new()
                    .selector("frontmatter")
                    .scope("observation")
                    .missing(&["controls", "exact-result-artifacts", "assessment"]),
            )?;
            ...
            let rationale = sections
                ...
                .map(str::to_string)
                .unwrap_or_else(|| {
                    "Exact source outcome retained; no independent assessment.".to_string()
                });
            let assessment_payload = json!({
    ...
    fn database(&mut self, path: &Path) -> Result<(), ImportError> {
        ...
        for (source, target) in [
            (path.to_path_buf(), dest.clone()),
            (sidecars[0].clone(), dest_wal.clone()),
        ] {
            if source.exists() {
        ...
        self.item(
            ...
            vec![(
                ...
                "All user table definitions retained; every row separately inventoried."
                    .to_string(),
            )],
        );
        for (name, _sql) in &tables {
            ...
            while let Some(row) = rows.next()? {
                ...
                let has_nonfinite = column_names.iter().enumerate().any(
                    |(i, _)| matches!(row.get_ref(i), Ok(ValueRef::Real(f)) if !f.is_finite()),
                );
                        ...
                        .is_some_and(|s| !s.is_empty())
                {
                    ...
                    let hypothesis = raw
                        ...
                        .unwrap_or_default()
                        .to_string();
                    let prefix = if name == "research_intents" {
                        "research-journal:"
                    } else {
                        "journal:"
                    };
                    ...
                    let rec = self.record(
                        ...
                        RecordOpts::new()
                            .selector(loc.clone())
                            .scope("observation")
                            .missing(&["verified-freeze", "verdict"]),
                    )?;
                    ...
                    let key = if name == "research_outcomes" {
                        "experiment_id"
                    } else {
                        "trade_id"
                    };
                    if let Some(idv) = raw.get(key).and_then(Value::as_str) {
                        let prefix = if name == "research_outcomes" {
                            "research-journal:"
                        } else {
                            "journal:"
                        };
                        ...
                        let rec = self.record(
                            ...
                            RecordOpts::new()
                                .selector(loc.clone())
                                .scope("observation")
                                .missing(&["protocol", "controls", "assessment"]),
                        )?;
                        self.item(
                            path,
                            &loc,
                        ...
                        self.item(
                            ...
                            vec![(
                                ...
                                "Unknown table/row shape retained without invented mapping."
                                    .to_string(),
                            )],
                        );
                    }
    ...
    fn run(&mut self, paths: Vec<PathBuf>, patterns: Vec<String>) -> Result<Value, ImportError> {
        ...
        for path in &paths {
            ...
            let result: Result<(), ImportError> = (|| {
                if db_suffixes.contains(&ext.as_str()) {
                    ...
                    match orbit_research_common::legacy_contract::parse_json(&bytes) {
        ...
        if self.adapter == "parallax" && !any_sqlite {
            ...
            self.item(
                ...
                vec![(
                    ...
                    "No SQLite journal selected/discovered; no intent/outcome history invented."
                        .to_string(),
                )],
            );
        }
        ...
        if !collisions.is_empty() {
            self.records.retain(|r| {
                !r.get("id")
                    .and_then(Value::as_str)
                    .is_some_and(|id| collisions.contains(id))
            });
            for item in self.inventory.iter_mut() {
                let candidate_ids = item["candidate_ids"]
                    .as_array()
                    .cloned()
                    .unwrap_or_default();
        ...
        let selected_files: HashSet<String> = self
            ...
            .collect();
        let before_snapshot: Vec<(PathBuf, Option<String>)> = self
            .before
            .iter()
...
pub fn import_source(
    ...
    if !root.is_dir() || !ADAPTERS.contains(&adapter) {
        return Err(value_error(
            "source must be a directory and adapter must be supported",
        ));
    }
    repository.parse::<RepositoryId>().map_err(|_| {
        value_error("repository identity must be an explicit stable namespace, not a path")
    })?;
...
pub fn write_report(
    ...
    for root in source_roots {
        ...
        if resolved.starts_with(&root_resolved) {
            return Err(value_error(
                "report output must be outside every source root",
            ));
        }
    }
