...
use orbit_research_common::legacy_contract::validate;
...
fn all_four_adapters_cover_the_synthetic_fixture() {
    ...
    for adapter in orbit_research_core::legacy_import::ADAPTERS {
        let root = tmp.path().join(adapter);
        let report = import_source(&root, adapter, adapter, None, None)
            .unwrap_or_else(|e| panic!("{adapter} import failed: {e}"));
        ...
        assert!(
            errors.is_empty(),
            "{adapter} report failed contract validation: {errors:?}"
        );
...
fn parallax_fixture_covers_table_row_and_sqlite_journal() {
    ...
    assert!(
        ids.iter()
            .any(|id| id.starts_with("urn:research:parallax:claim:journal%3Aintent-1"))
    );
    assert!(
        ids.iter()
            .any(|id| id.starts_with("urn:research:parallax:experiment:journal%3Aintent-1"))
    );
    ...
    assert!(
        !ids.iter()
            .any(|id| id.starts_with("urn:research:parallax:protocol:"))
    );
}
...
fn orrery_fixture_maps_sim_catalog_and_retains_result_artifact() {
    let tmp = tempfile::tempdir().expect("tempdir");
    create_fixture(tmp.path());
    let report = import_source(&tmp.path().join("orrery"), "orrery", "orrery", None, None)
        .expect("import succeeds");
    let ids = candidate_ids(&report);
    assert!(ids.contains(&"urn:research:orrery:program:control".to_string()));
    assert!(
        ids.iter()
            .any(|id| id.starts_with("urn:research:orrery:artifact:"))
    );
}
...
fn dry_run_never_writes_and_write_report_refuses_unsafe_destinations() {
    ...
    for (path, bytes) in &before {
        assert_eq!(
            ...
            bytes,
            "import must not mutate source bytes"
        );
    }
...
fn duplicate_json_keys_and_non_finite_numbers_are_parse_errors() {
    ...
    fs::write(root.join("gates/duplicate.json"), r#"{"id":"a","id":"b"}"#)
        .expect("write duplicate-key file");
    fs::write(
        root.join("gates/nonfinite.json"),
        r#"{"id":"c","value":NaN}"#,
    )
    .expect("write non-finite file");
    ...
    for path in ["gates/duplicate.json", "gates/nonfinite.json"] {
        ...
        assert_eq!(
            codes,
            vec!["read-error"],
            "{path} should fail to parse, not silently coerce"
        );
    }
}
