...
use crate::legacy_index::build::{key_of, pin};
use crate::legacy_index::config::{guard_output, load_config};
use crate::legacy_index::read::read_index;
use crate::legacy_index::{IndexError, Result, require, text};
...
pub fn export_browser(database: &Path, output: &Path, config_path: &Path) -> Result<ExportOutcome> {
    ...
    let canonical = orbit_research_common::legacy_contract::canonical_json(&projection)?;
...
fn fill_media_entry(
    ...
    let record_key = entry
        .get("record")
        .and_then(Value::as_str)
        .unwrap_or_default()
        .to_owned();
    require(
        nodes.contains(&record_key),
        "media record pin is absent from index",
    )?;
    let role = entry
        .get("role")
        .and_then(Value::as_str)
    ...
    require(
        matches!(
            role.as_str(),
            "illustration" | "empirical-evidence" | "simulation"
        ),
    ...
    require(
        crate::legacy_index::digest_bytes(&data) == expected_sha256,
        "media digest differs from exact Git snapshot",
    )?;
    require(
        path.is_file() && crate::legacy_index::file_digest(&path)? == expected_sha256,
    ...
    if raster && role != "simulation" {
        let digest_hex = expected_sha256
            .strip_prefix("sha256:")
