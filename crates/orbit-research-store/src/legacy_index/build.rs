...
use crate::legacy_owner::Checkout;
use orbit_research_common::legacy_contract::record_references;
use serde_json::{Value, json};

use crate::legacy_index::config::{load_config, valid_reference_shape};
use crate::legacy_index::{IndexConfig, IndexError, Problem, Result, require, text};
...
pub(crate) fn key_of(pin: &Pin4) -> Result<String> {
    let bytes = orbit_research_common::legacy_contract::canonical_json(&json!([
        pin.0, pin.1, pin.2, pin.3
    ]))?;
    Ok(crate::legacy_index::digest_bytes(&bytes)[7..].to_owned())
}
...
fn load_one_document(
    ...
    observations.insert(path.to_path_buf(), crate::legacy_index::digest_bytes(&data));
    let doc = orbit_research_common::legacy_contract::parse_json(&data)?;
    let kind = text(&doc, "kind").unwrap_or_default().to_owned();
    let mut errors = Vec::new();
    let (batch, manifests, inventory): (Vec<Value>, Vec<Value>, Option<Value>) = match kind.as_str()
    {
        ...
        "export" => {
            ...
            if !records_ok || !manifests_ok {
                ...
                if pinned != expected {
                    errors.push(
                        "export manifests do not account for exact record snapshots".to_owned(),
                    );
                }
                (batch, ms, None)
            }
    ...
    for record in &batch {
        errors.extend(orbit_research_common::legacy_contract::validate_structural(
            record,
        ));
    }
    ...
    for manifest in manifests {
        let valid: HashSet<(Option<&str>, Option<&str>)> = manifest
            ...
            .map(|repo| {
                (
                    text(repo, "id"),
                    repo.get("git_revision").and_then(Value::as_str),
                )
            })
            .collect();
...
fn build_nodes(
    ...
    for (record, source) in pairs {
        ...
        let canonical = String::from_utf8_lossy(
            &orbit_research_common::legacy_contract::canonical_json(record)?,
        )
        .into_owned();
        grouped
            .entry(k.clone())
    ...
    for (k, variants_map) in &grouped {
        ...
        if variants.len() > 1 {
            reasons.push(
                "conflicting records share this exact identity/revision/source pin".to_owned(),
            );
        }
        ...
        if let Some(code) = record_code(&r) {
            ...
            let ok = roots.get(repository).is_some_and(|root| {
                crate::legacy_owner::git::full_revision(revision)
    ...
    for k in &keys {
        ...
        if !historical && heads_set.len() > 1 {
            node.reasons
                .insert("conflicting revision heads require an owner decision".to_owned());
        }
    }
    ...
    let known: HashMap<crate::legacy_owner::RecordKey, Value> = derived
        .values()
        .map(|record| (crate::legacy_owner::record_key(record), record.clone()))
        .collect();
    for (k, record) in &derived {
        let mut extra = Vec::new();
        if record.get("schema_version").and_then(Value::as_u64) == Some(2) {
            extra.extend(crate::legacy_owner::native_reference_errors(record, &known));
        }
        ...
        {
            extra.extend(crate::legacy_owner::confirmation_errors(
                record,
                &known,
                &HashSet::new(),
            ));
    ...
    let mut pending: BTreeSet<Pin4> = nodes
        .iter()
        .filter(|(_, n)| {
            !n.reasons.is_empty() || n.edges.iter().any(|edge| edge.status == "pending")
        })
        .map(|(k, _)| k.clone())
        .collect();
    loop {
        ...
        for (k, n) in &nodes {
            if n.edges
                .iter()
                .any(|edge| pending.contains(&pin(&edge.reference)))
            {
                more.insert(k.clone());
            }
        }
        ...
        for edge in n.edges.iter_mut() {
            if pending.contains(&pin(&edge.reference)) {
                ...
                if edge.reason.is_empty() {
                    edge.reason =
                        "target has unresolved dependencies; inspect exact trace".to_owned();
                }
            }
        }
        n.reconciliation = if pending.contains(k) {
            "pending"
        } else {
            "resolved"
        };
    ...
    for (k, claim_pin) in claim_checks {
        ...
        let snapshot = nodes.get(&claim_pin).map(|claim| {
            (
                claim.history,
                text(&claim.record, "activity").map(str::to_owned),
            )
        });
            ...
            && (claim_history != "current" || claim_activity.as_deref() != Some("active"))
        {
            ...
            node.reasons
                .insert("assessment targets a historical, superseded or inactive claim".to_owned());
        }
    }

    // Competing eligible assessments do not silently adjudicate one another.
    let key_to_pin: HashMap<String, Pin4> = nodes
        .iter()
        .map(|(k, n)| (n.key.clone(), k.clone()))
        .collect();
    let claim_keys: Vec<Pin4> = nodes.keys().cloned().collect();
    for claim_key in &claim_keys {
        let assessment_keys = nodes
            .get(claim_key)
            .map(|n| n.assessments.clone())
            .unwrap_or_default();
        let eligible: Vec<Pin4> = assessment_keys
            ...
            .filter(|assessment_pin| {
                nodes
                    .get(*assessment_pin)
                    .is_some_and(|n| n.confirmation == "eligible")
            })
            .cloned()
            .collect();
        ...
        if verdicts.len() > 1 {
            for assessment_pin in &eligible {
                if let Some(n) = nodes.get_mut(assessment_pin) {
                    ...
                    n.reasons
                        .insert("current assessments disagree on this exact claim pin".to_owned());
                }
            }
        }
    }
        ...
        for (k, n) in nodes.iter_mut() {
            for edge in n.edges.iter_mut() {
                if conflicts.contains(&pin(&edge.reference)) {
                    edge.status = "pending";
                    edge.reason =
                        "target has conflicting assessments in its evidence closure".to_owned();
    ...
    unresolved.sort_by_key(|value| {
        orbit_research_common::legacy_contract::canonical_json(value).unwrap_or_default()
    });
    ...
    for (path, digest) in observations.iter() {
        if &crate::legacy_index::file_digest(path)? != digest {
    ...
    manifests_sorted.sort_by_key(|value| {
        orbit_research_common::legacy_contract::canonical_json(value).unwrap_or_default()
    });
    ...
    let digest = crate::legacy_index::digest_bytes(
        &orbit_research_common::legacy_contract::canonical_json(&result)?,
    );
...
fn record_code(record: &Value) -> Option<&Value> {
    ...
    payload
        ...
        .or_else(|| {
            payload
                .get("semantic")
                .and_then(|semantic| semantic.get("code"))
                .filter(|value| !value.is_null())
        })
}
...
fn source_state(
    ...
    let repository = text(&provenance, "repository")
        .unwrap_or_default()
        .to_owned();
    let Some(root) = roots.get(&repository) else {
        return vec!["owner checkout is not mapped".to_owned()];
    };
    let git_revision = text(&provenance, "git_revision")
        .unwrap_or_default()
        .to_owned();
    let working_tree = provenance
        .get("working_tree")
        .and_then(Value::as_bool)
        .unwrap_or(false);
    if git_revision.is_empty() || working_tree {
        return vec![
            "source is unpinned or marked working-tree; owner reconciliation required".to_owned(),
        ];
    }
    ...
    if !cache.contains_key(&identity) {
        let blob_oid = provenance
            .get("blob_oid")
            .and_then(Value::as_str)
            .map(str::to_owned);
        let reasons = match source_check(
            root,
            &path,
            &git_revision,
            blob_oid.as_deref(),
...
fn source_check(
    ...
    if !safe.is_file() {
        ...
        let digest = crate::legacy_index::file_digest(&safe)?;
...
fn verify_native_snapshot(
    ...
    let mut committed = orbit_research_common::legacy_contract::parse_json(&data)?;
    require(
        orbit_research_common::legacy_contract::validate_structural(&committed).is_empty(),
        "invalid pinned record",
    )?;
    crate::legacy_owner::NativeReceipt::new(&committed, path).verify_chain(checkout, revision)?;
    let blob_oid = checkout.text(&["rev-parse", &format!("{revision}:{path}")]);
    if let Some(object) = committed
        .get_mut("provenance")
        .and_then(Value::as_object_mut)
    {
        ...
        object.insert(
            "sha256".into(),
            json!(crate::legacy_index::digest_bytes(&data)),
        );
    ...
    require(
        orbit_research_common::legacy_contract::canonical_json(&committed)?
            == orbit_research_common::legacy_contract::canonical_json(record)?,
...
fn artifact_state(
    ...
    if locator.contains(':') || locator.contains('\\') {
        return vec![
            "artifact locator requires explicit owner verification; no automatic fetch".to_owned(),
        ];
    }
    ...
    let digest = match crate::legacy_index::file_digest(&path) {
        Ok(digest) => digest,
        Err(error) => return vec![error.to_string()],
    };
    ...
    if digest != expected {
        return vec![
            "artifact bytes differ from snapshot digest (or require a typed owner verifier)"
                .to_owned(),
        ];
    }
    vec![]
