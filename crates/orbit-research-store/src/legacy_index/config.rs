...
use crate::legacy_owner::{Checkout, SubprocessGit};
use orbit_research_common::legacy_contract::RepositoryId;
...
pub fn load_config(path: &Path) -> Result<IndexConfig> {
    ...
    let config = orbit_research_common::legacy_contract::parse_json(&fs::read(&path)?)?;
...
pub fn guard_output(
    ...
    require(
        !has_symlink_component(output),
        "output may not use symlinks",
    )?;
