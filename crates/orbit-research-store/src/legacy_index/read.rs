...
use crate::legacy_index::{IndexError, Result, require};
...
pub fn read_index(database: &Path) -> Result<Value> {
    ...
    let body: String = conn.query_row("SELECT body FROM projection WHERE id = 1", [], |row| {
        row.get(0)
    })?;
    let mut result = orbit_research_common::legacy_contract::parse_json(body.as_bytes())?;
    let digest = result
        .as_object_mut()
        .and_then(|object| object.remove("content_digest"))
        .and_then(|value| value.as_str().map(str::to_owned))
        .ok_or_else(|| IndexError::Invalid("invalid projection body".into()))?;
    let recomputed = crate::legacy_index::digest_bytes(
