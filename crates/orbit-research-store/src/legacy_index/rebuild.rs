...
use crate::legacy_index::build::build_projection;
use crate::legacy_index::config::{guard_output, load_config};
use crate::legacy_index::{IndexError, Result};
...
pub fn rebuild(config_path: &Path, database: &Path) -> Result<RebuildOutcome> {
    ...
    let output = guard_output(
        database,
        &initial.roots,
        &inputs_of(config_path, &initial.paths),
    )?;
    let parent = output
        .parent()
    ...
    let output = guard_output(
        &output,
        &config.roots,
        &inputs_of(config_path, &config.paths),
    )?;
...
fn canonical_text(value: &Value) -> Result<String> {
    Ok(
        String::from_utf8_lossy(&orbit_research_common::legacy_contract::canonical_json(
            value,
        )?)
        .into_owned(),
    )
