...
use crate::legacy_owner::git::{Git, full_revision};
use crate::legacy_owner::{Result, require};
...
impl Checkout {
    ...
    pub fn open(path: &Path, git: Arc<dyn Git>) -> Result<Self> {
        let root = path.canonicalize().map_err(|error| {
            crate::legacy_owner::OwnerError::Invalid(format!("{}: {error}", path.display()))
        })?;
    ...
    pub fn head(&self) -> Result<String> {
        self.text(&["rev-parse", "HEAD"]).ok_or_else(|| {
            crate::legacy_owner::OwnerError::Invalid("owner requires an initial Git commit".into())
        })
    }
    ...
    pub fn git_bytes(&self, revision: &str, path: &str) -> Result<Vec<u8>> {
        ...
        let bytes = self
            ...
            .ok_or_else(|| {
                crate::legacy_owner::OwnerError::Invalid(format!(
                    "no committed source at {revision}:{path}"
                ))
            })?;
...
pub fn safe_path(root: &Path, relative: &str) -> Result<PathBuf> {
    ...
    for component in candidate.components() {
        match component {
            ...
            Component::CurDir => {}
            _ => {
                return Err(crate::legacy_owner::OwnerError::Invalid(
