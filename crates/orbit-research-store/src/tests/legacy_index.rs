...
fn build_record(
    ...
    record
        ...
        .remove("revision_id");
    let digest =
        orbit_research_common::legacy_contract::revision_digest(&record).expect("revision digest");
...
fn commit_records(root: &Path, records: &mut [Value]) {
    ...
    for record in records.iter() {
        let relative = record["provenance"]["path"]
            .as_str()
            .expect("path")
            .to_owned();
        let bytes =
            orbit_research_common::legacy_contract::canonical_json(record).expect("canonical json");
...
fn atomic_rebuild_and_invalid_rebuild_retains_previous_database() {
    ...
    let head = program["provenance"]["git_revision"]
        .as_str()
        .expect("head")
        .to_owned();
    ...
    let outcome =
        crate::legacy_index::rebuild(&config_path, &database).expect("first rebuild succeeds");
    assert_eq!(1, outcome.records);
    assert_eq!(0, outcome.pending);

    let published = crate::legacy_index::read_index(&database).expect("read published index");
    ...
    let error = crate::legacy_index::rebuild(&config_path, &database)
        .expect_err("invalid rebuild is refused");
    assert!(
        error
            .problems()
    ...
    let still_published =
        crate::legacy_index::read_index(&database).expect("previous index remains readable");
    assert_eq!(
        published, still_published,
        "invalid rebuild must not touch the previous database"
    );
}
...
fn pending_vs_resolved_pins() {
    ...
    let program_head = program["provenance"]["git_revision"]
        .as_str()
        .expect("head")
        .to_owned();
    let documents = scratch.join("documents");
    ...
    let config_path = scratch.join("index-config.json");
    let database = scratch.join("index.sqlite");
    let outcome = crate::legacy_index::rebuild(&config_path, &database).expect("rebuild succeeds");
    assert_eq!(3, outcome.records);
    assert_eq!(1, outcome.pending, "only the dangling claim is pending");

    let projection = crate::legacy_index::read_index(&database).expect("read index");
    ...
    let trace = crate::legacy_index::trace(&database, pending_node["key"].as_str().expect("key"))
