...
use crate::legacy_owner::{Owner, OwnerConfig, reference};
...
fn competing_eligible_assessments_do_not_silently_confirm() {
    ...
    let seed_digest = write_local_artifact(
        &root,
        "data/seed-plan.bin",
        b"Fixture seed plan; no scientific record.\n",
    );
    ...
    let data = owner
        .apply(
            ...
            &request(
                "seed-plan",
                dataset_payload(&seed_digest, "data/seed-plan.bin"),
            ),
        )
        .expect("dataset");
    ...
    let supported_reference = reference(
        &owner
            .pin(
                supported["id"].as_str().expect("id"),
                supported["revision_id"].as_str().expect("revision"),
                &head,
            )
    ...
    let refuted_reference = reference(
        &owner
            .pin(
                refuted["id"].as_str().expect("id"),
                refuted["revision_id"].as_str().expect("revision"),
                &head,
            )
    ...
    std::fs::write(
        documents.join("export.json"),
        serde_json::to_vec(&bundle).expect("bytes"),
    )
    ...
    std::fs::write(&config_path, serde_json::to_vec(&config).expect("bytes"))
        .expect("write config");

    let database = scratch.join("index.sqlite");
    crate::legacy_index::rebuild(&config_path, &database).expect("rebuild succeeds");
